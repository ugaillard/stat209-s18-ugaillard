...
```{r}
ages_30_49$fourties <- ages_30_49$age >= 40
ages_30_49
```
...
```{r}
new_ctry <- fct_lump(pf$country, n = 12)
ggplot(pf, aes(new_ctry)) +
  geom_bar()
```
...
```{r}
newer_ctry <- fct_lump(pf$country, n = 5)
model <- lm_basic(self_reliance ~ 1 + newer_ctry, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: The slow is statistically significantly different. This could be an interest question to address since maybe most people really mask their true emotions and feelings in these types of tests to appear as better versions of themselves.

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model <- lm_basic(self_reliance ~ 1 + gender + baseline, data = pf)
reg_table(model, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: Males tend to be less self-reliant than females.
